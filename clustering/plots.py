    ...
    with _chart_lock:
        ...
        for c in range(n_clusters):
            ...
            ax.scatter(X_pca[mask, 0], X_pca[mask, 1],
                       c=PALETTE[c], alpha=0.55, s=40, label=f'Clúster {c+1}', edgecolors='none')
...
def centroids_chart(centroids_orig, n_clusters):
    ...
    with _chart_lock:
        ...
        for i in range(n_clusters):
            ...
            ax.bar(x + offset, normalized[i], width, label=f'Clúster {i+1}',
...
def distribution_chart(labels, n_clusters):
    ...
    with _chart_lock:
        ...
        bars = ax.bar(
            [f'Clúster {c+1}' for c in range(n_clusters)],
